      ...
      <div className="mt-1 flex flex-col">
        <div className="hidden sm:block" >
          <div href="#" className="flex item-center inset-0">
            <img src="../src/assets/hero.png" alt="" className='w-full h-48 sm:h-64 md:h-80 lg:h-96 object-fill'/> 
          </div>
        </div>
        ...
        {events.length > 0 && events.map((event) => {
          ...
          if (eventDate > currentDate || eventDate.toDateString() === currentDate.toDateString()){
            return (
              ...
              {event.image && (
                <img
                  src={event.image.startsWith('data:') ? event.image : `${import.meta.env.VITE_API_URL || 'http://localhost:4000'}/${event.image}`}
                  alt={event.title}
                  width="300" 
                  height="200" 
                ...
                <img src="../src/assets/event.jpg" alt="" className='rounded-tl-[0.75rem] rounded-tr-[0.75rem] rounded-br-[0] rounded-bl-[0] object-fill aspect-16:9'/> 
